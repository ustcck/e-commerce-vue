    ...
    <div>
        ...
        <div class="table-responsive" v-if="orderItems && orderItems.length > 0">
            <table class="table table-striped">
                ...
                <tr>
                    ...
                    <th v-on:click="changeOrder('quantity')"><span v-text="$t('storeApp.orderItem.quantity')">Quantity</span> <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'quantity'"></jhi-sort-indicator></th>
                    <th v-on:click="changeOrder('totalPrice')"><span v-text="$t('storeApp.orderItem.totalPrice')">Total Price</span> <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'totalPrice'"></jhi-sort-indicator></th>
                    <th v-on:click="changeOrder('status')"><span v-text="$t('storeApp.orderItem.status')">Status</span> <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'status'"></jhi-sort-indicator></th>
                    <th v-on:click="changeOrder('product.name')"><span v-text="$t('storeApp.orderItem.product')">Product</span> <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'product.name'"></jhi-sort-indicator></th>
                    <th v-on:click="changeOrder('order.code')"><span v-text="$t('storeApp.orderItem.order')">Order</span> <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'order.code'"></jhi-sort-indicator></th>
                    <th></th>
                </tr>
                </thead>
                <tbody>
                <tr v-for="orderItem in orderItems"
                    ...
                    <td>
                        <div v-if="orderItem.order">
                            <router-link :to="{name: 'ProductOrderView', params: {productOrderId: orderItem.order.id}}">{{orderItem.order.code}}</router-link>
                        </div>
                    </td>
                    <td class="text-right">
                        <div class="btn-group">
        ...
        <b-modal ref="removeEntity" id="removeEntity" >
            ...
            <div class="modal-body">
                <p id="jhi-delete-orderItem-heading" v-text="$t('storeApp.orderItem.delete.question', {'id': removeId})">Are you sure you want to delete this Order Item?</p>
            </div>
